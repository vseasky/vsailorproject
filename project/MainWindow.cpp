...
/**
 * Function returns a features string with closable (c), movable (m) and
 * floatable (f) features. i.e. The following string is for a not closable but
 * movable and floatable widget: c- m+ f+
 */

// 函数返回一个具有可关闭 (c)、可移动 (m) 和可浮动 (f)
// 特征的特征字符串。即以下字符串适用于不可关闭但可移动且可浮动的小部件： cm- m+
// f+
 ...
 */
// 将 featuresString() 返回的字符串附加到给定 DockWidget 的窗口标题
...
public:
	...
	createDockAreaTitleBar(ads::CDockAreaWidget *DockArea) const override
	{
		...
		int Index =
				TitleBar->indexOf(TitleBar->button(ads::TitleBarButtonTabsMenu));
 ...
 */
// 自定义QTableWidget，带有最小尺寸提示，用于测试CDockWidget的setMinimumSizeHintMode()函数
...
struct MainWindowPrivate
{
	...
	ads::CDockWidget *createFileSystemTreeDockWidget()
	{
		...
		ads::CDockWidget *DockWidget =
				new ads::CDockWidget(QString("Filesystem %1").arg(FileSystemCount++));
	 ...
	 */
	// 使用 QCalendarWidget 创建停靠小部件
	ads::CDockWidget *createCalendarDockWidget()
	{
		static int CalendarCount = 0;
		QCalendarWidget *w = new QCalendarWidget();
		ads::CDockWidget *DockWidget =
				new ads::CDockWidget(QString("Calendar %1").arg(CalendarCount++));
		// The following lines are for testing the setWidget() and takeWidget()
		// functionality
		DockWidget->setWidget(w);
		DockWidget->setWidget(
				w); // what happens if we set a widget if a widget is already set
		DockWidget->takeWidget(); // we remove the widget
		DockWidget->setWidget(
				w); // and set the widget again - there should be no error
	...
	ads::CDockWidget *createLongTextLabelDockWidget()
	{
		...
		l->setText(
				QString(
						"Label %1 %2 - Lorem ipsum dolor sit amet, consectetuer adipiscing "
						"elit. "
		...
		ads::CDockWidget *DockWidget =
				new ads::CDockWidget(QString("Label %1").arg(LabelCount++));
	...
	ads::CDockWidget *createEditorWidget()
	{
		...
		ads::CDockWidget *DockWidget =
				new ads::CDockWidget(QString("Editor %1").arg(EditorCount++));
		DockWidget->setWidget(w);
	...
	ads::CDockWidget *createImageViewer()
	{
		...
		{
			case 0:
				FileName = ":adsdemo/images/ads_tile_blue.svg";
				break;
			case 1:
				FileName = ":adsdemo/images/ads_tile_blue_light.svg";
				break;
			case 2:
				FileName = ":adsdemo/images/ads_tile_green.svg";
				break;
			case 3:
				FileName = ":adsdemo/images/ads_tile_orange.svg";
				break;
		}
		
		auto Result = w->loadFile(FileName);
		qDebug() << "loadFile result: " << Result;
		ads::CDockWidget *DockWidget = new ads::CDockWidget(
				QString("Image Viewer %1").arg(ImageViewerCount++));
	...
	ads::CDockWidget *createTableWidget()
	{
		...
		ads::CDockWidget *DockWidget =
				new ads::CDockWidget(QString("Table %1").arg(TableCount++));
		...
		for (int col = 0; col < colCount; ++col)
		{
			w->setHorizontalHeaderItem(
					col, new QTableWidgetItem(QString("Col %1").arg(col + 1)));
			for (int row = 0; row < rowCount; ++row)
			{
				w->setItem(
						row, col,
						new QTableWidgetItem(QString("T %1-%2").arg(row + 1).arg(col + 1)));
			}
		}
		DockWidget->setWidget(w);
		DockWidget->setIcon(svgIcon(":/adsdemo/images/grid_on.svg"));
		DockWidget->setMinimumSizeHintMode(
				ads::CDockWidget::MinimumSizeHintFromContent);
		auto ToolBar = DockWidget->createDefaultToolBar();
		auto Action = ToolBar->addAction(svgIcon(":/adsdemo/images/fullscreen.svg"),
		                                 "Toggle Fullscreen");
	 ...
	 */
	// 创建 QQuickWidget 用于测试 OpenGL 和 QQuick
	 ...
	 */
	// 在 Windows 上创建 ActiveX 小部件
	ads::CDockWidget *createActiveXWidget(QWidget *parent = nullptr)
	{
		static int ActiveXCount = 0;
		QAxWidget *w =
				new QAxWidget("{6bf52a52-394a-11d3-b153-00c04f79faa6}", parent);
		ads::CDockWidget *DockWidget =
				new ads::CDockWidget(QString("Active X %1").arg(ActiveXCount++));
		DockWidget->setWidget(w);
		ui.menuView->addAction(DockWidget->toggleViewAction());
		return DockWidget;
	}
...
void MainWindowPrivate::createContent()
{
	...
	// For this Special Dock Area we want to avoid dropping on the center of it
	// (i.e. we don't want this widget to be ever tabbified):
	// 对于这个特殊的停靠区域，我们希望避免落在它的中心（即我们不希望这个小部件被选项卡化）：
	{
		// SpecialDockArea->setAllowedAreas(ads::OuterDockAreas);
		SpecialDockArea->setAllowedAreas(
				{
	...
	// 测试自定义工厂 - 我们在标题栏中注入一个帮助按钮
	ads::CDockComponentsFactory::setFactory(new CCustomComponentsFactory());
	auto TopDockArea =
			DockManager->addDockWidget(ads::TopDockWidgetArea, FileSystemWidget);
	// Uncomment the next line if you would like to test the
	// 如果您想测试，请取消注释下一行
	// HideSingleWidgetTitleBar functionality
	// 隐藏SingleWidgetTitleBar功能
	// TopDockArea->setDockAreaFlag(ads::CDockAreaWidget::HideSingleWidgetTitleBar,
	// true);
	ads::CDockComponentsFactory::resetDefaultFactory();
	
	// We create a calendar widget and clear all flags to prevent the dock area
	// 我们创建一个日历小部件并清除所有标志以防止停靠区域
	// from closing
	DockWidget = createCalendarDockWidget();
	DockWidget->setTabToolTip(QString("Tab ToolTip\nHodie est dies magna"));
	auto DockArea = DockManager->addDockWidget(ads::CenterDockWidgetArea,
	                                           DockWidget, TopDockArea);
	// Now we create a action to test resizing of DockArea widget
	// 现在我们创建一个操作来测试 DockArea 小部件的大小调整
	auto Action = ui.menuTests->addAction(
			QString("Resize %1").arg(DockWidget->windowTitle()));
	QObject::connect(Action, &QAction::triggered, [DockArea]()
	{
		// Resizing only works, if the Splitter is visible and has a valid
		// 仅当拆分器可见且具有有效的情况下，调整大小才有效
		// sizes
	...
	Action = ui.menuTests->addAction(
			QString("Set %1 Floating").arg(DockWidget->windowTitle()));
	DockWidget->connect(Action, SIGNAL(triggered()), SLOT(setFloating()));
	Action = ui.menuTests->addAction(
			QString("Set %1 As Current Tab").arg(DockWidget->windowTitle()));
	DockWidget->connect(Action, SIGNAL(triggered()), SLOT(setAsCurrentTab()));
	Action = ui.menuTests->addAction(
			QString("Raise %1").arg(DockWidget->windowTitle()));
...
#ifdef Q_OS_WIN
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
	DockManager->addDockWidget(ads::CenterDockWidgetArea, createActiveXWidget(),
	                           RighDockArea);
#endif
#endif
	...
	for (auto DockWidget: DockManager->dockWidgetsMap())
	{
		...
		_this->connect(DockWidget, SIGNAL(visibilityChanged(bool)),
		               SLOT(onViewVisibilityChanged(bool)));
	}
...
void MainWindowPrivate::createActions()
{
	...
	SavePerspectiveAction = new QAction("创造视角", _this);
	SavePerspectiveAction->setIcon(
			svgIcon(":/adsdemo/images/picture_in_picture.svg"));
	_this->connect(SavePerspectiveAction, SIGNAL(triggered()),
	...
	QAction *a = ui.toolBar->addAction("创建浮动编辑器");
	a->setProperty("Floating", true);
	a->setToolTip("Creates floating dynamic dockable editor windows that are "
	              "deleted on close");
	a->setIcon(svgIcon(":/adsdemo/images/note_add.svg"));
	_this->connect(a, SIGNAL(triggered()), SLOT(createEditor()));
	ui.menuTests->addAction(a);
	
	a = ui.toolBar->addAction("创建停靠编辑器");
	...
	a = ui.toolBar->addAction("创建编辑器选项卡");
	a->setProperty("Floating", false);
	a->setToolTip(
			"Creates a editor tab and inserts it as second tab into an area");
	a->setIcon(svgIcon(":/adsdemo/images/tab.svg"));
	a->setProperty("Tabbed", true);
	_this->connect(a, SIGNAL(triggered()), SLOT(createEditor()));
	ui.menuTests->addAction(a);
	
	a = ui.toolBar->addAction("创建浮动表");
	a->setToolTip(
	...
	a = ui.toolBar->addAction("创建图像查看器");
	...
	a = Menu->addAction("浮动图像查看器");
	_this->connect(a, SIGNAL(triggered()), SLOT(createImageViewer()));
	a = Menu->addAction("停靠的图像查看器");
	_this->connect(a, SIGNAL(triggered()), SLOT(createImageViewer()));
	a = Menu->addAction("固定图像查看器");
	_this->connect(a, SIGNAL(triggered()), SLOT(createImageViewer()));
	
	ui.menuTests->addSeparator();
	a = ui.menuTests->addAction("显示状态对话框");
	_this->connect(a, SIGNAL(triggered()), SLOT(showStatusDialog()));
	
	a = ui.menuTests->addAction("切换标签 0 窗口标题");
	_this->connect(a, SIGNAL(triggered()), SLOT(toggleDockWidgetWindowTitle()));
	ui.menuTests->addSeparator();
	
	a = ui.toolBar->addAction("应用 VS 风格");
	a->setToolTip(
			"Applies a Visual Studio light style (visual_studio_light.css).");
		...
		: QMainWindow(parent), d(new MainWindowPrivate(this))
{
	...
	// 如果选项卡关闭按钮应该是，请取消注释以下行
	// a QToolButton instead of a QPushButton
	// CDockManager::setConfigFlags(CDockManager::configFlags() |
	// CDockManager::TabCloseButtonIsToolButton);
	...
	// 如果您想要固定的制表符宽度，请取消注释以下行
	// not change if the visibility of the close button changes
	// CDockManager::setConfigFlag(CDockManager::RetainTabSizeWhenCloseButtonHidden,
	// true);
	
	// uncomment the following line if you don't want close button on DockArea's
	// title bar 如果您不想在 DockArea 标题栏上显示关闭按钮，请取消注释以下行
	// CDockManager::setConfigFlag(CDockManager::DockAreaHasCloseButton, false);
	
	// uncomment the following line if you don't want undock button on DockArea's
	// title bar 如果您不想在 DockArea 标题栏上取消停靠按钮，请取消注释以下行
	// CDockManager::setConfigFlag(CDockManager::DockAreaHasUndockButton, false);
	
	// uncomment the following line if you don't want tabs menu button on
	// DockArea's title bar 如果您不想在 DockArea
	// 的标题栏上显示选项卡菜单按钮，请取消注释以下行
	// CDockManager::setConfigFlag(CDockManager::DockAreaHasTabsMenuButton,
	// false);
	
	// uncomment the following line if you don't want disabled buttons to appear
	// on DockArea's title bar 如果您不希望禁用按钮出现在 DockArea
	// 的标题栏上，请取消注释以下行
	// CDockManager::setConfigFlag(CDockManager::DockAreaHideDisabledButtons,
	// true);
	
	// uncomment the following line if you want to show tabs menu button on
	// DockArea's title bar only when there are more than one tab and at least of
	// them has elided title
	// 如果您想仅当有多个选项卡且至少其中一个已省略标题时才在 DockArea
	// 的标题栏上显示选项卡菜单按钮，请取消注释以下行
	// CDockManager::setConfigFlag(CDockManager::DockAreaDynamicTabsMenuButtonVisibility,
	// true);
	
	// uncomment the following line if you want floating container to always show
	// application title instead of active dock widget's title
	// 如果您希望浮动容器始终显示应用程序标题而不是活动停靠小部件的标题，请取消注释以下行
	// CDockManager::setConfigFlag(CDockManager::FloatingContainerHasWidgetTitle,
	// false);
	
	// uncomment the following line if you want floating container to show active
	// dock widget's icon instead of always showing application icon
	// 如果您希望浮动容器显示活动停靠小部件的图标而不是始终显示应用程序图标，请取消注释以下行
	// CDockManager::setConfigFlag(CDockManager::FloatingContainerHasWidgetIcon,
	// true);
	
	// uncomment the following line if you want a central widget in the main dock
	// container (the dock manager) without a titlebar
	// 如果您希望主停靠容器（停靠管理器）中的中央小部件不带标题栏，请取消注释以下行
	// If you enable this code, you can test it in the demo with the Calendar 0
	// 如果启用此代码，您可以在演示中使用 Calendar 0 进行测试
	// dock widget.
	// CDockManager::setConfigFlag(CDockManager::HideSingleCentralWidgetTitleBar,
	// true);
	...
	// uncomment if you would like to close tabs with the middle mouse button, web
	// browser style 如果您想使用鼠标中键（网络浏览器样式）关闭选项卡，请取消注释
...
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
	connect(d->PerspectiveComboBox, SIGNAL(activated(QString)), d->DockManager,
	        SLOT(openPerspective(QString)));
#else
...
void CMainWindow::createEditor()
{
	...
	if (EditorArea)
	{
		...
		{
			d->DockManager->setConfigFlag(ads::CDockManager::EqualSplitOnInsertion,
			                              true);
